...
export class TableService {
  ...
  getAll(): Observable<any> {
    ...
    return this.http.get(`${this.apiUrl}?branchId=${branchId}`, this.getHeaders());
  }

  create(table: { table_number: number, status: string }): Observable<any> {
    const branchId = this.branchService.getBranchId(); // 🔥 Ajustado
    const payload = { ...table, branchId };
    return this.http.post(this.apiUrl, payload, this.getHeaders());
  }

  updateStatus(id: number, status: string): Observable<any> {
    return this.http.put(`${this.apiUrl}/status/${id}`, { status }, this.getHeaders());
  }
  ...
  getTableById(id: number): Observable<any> {
    return this.http.get(`http://localhost:3000/api/tables/${id}`, this.getHeaders());
  }
